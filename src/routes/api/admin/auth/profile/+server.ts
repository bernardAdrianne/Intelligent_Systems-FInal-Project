...
export const GET: RequestHandler = async ({ request }) => {
    ...
    try {
        ...
        if (adminProfile.length === 0) {
            return json({ success: false, message: 'Admin not found.' }, { status: 404 });
        }

        return json({ success: true, data: adminProfile[0] }, { status: 200 });
...
export const PUT: RequestHandler = async ({ request }) => {
    ...
    try {
        ...
        return json({ success: true, message: 'Profile Updated successfully.', data: updatedAdmin });
    } catch (error: any) {
